...
class VGGnet(nn.Module):
    ...
    def forward(self,x):
        ...
        # Transposed Convolutional Layers (Decoder - Upsampling)
        xu5 = self.upconv5(x)
        xu51 = self.pad_crop_concat(xu5, x)  # Updated concatenation method
        xd51 = F.relu(self.d51(xu51))
        xd52 = F.relu(self.d52(xd51))

        xu4 = self.upconv4(xd52)
        xu41 = self.pad_crop_concat(xu4, xu51)  # Updated concatenation method
        xd41 = F.relu(self.d41(xu41))
        xd42 = F.relu(self.d42(xd41))

        xu3 = self.upconv3(xd42)
        xu31 = self.pad_crop_concat(xu3, xu41)  # Updated concatenation method
        xd31 = F.relu(self.d31(xu31))
        xd32 = F.relu(self.d32(xd31))

        xu2 = self.upconv2(xd32)
        xu21 = self.pad_crop_concat(xu2, xu31)  # Updated concatenation method
    ...
    def pad_crop_concat(self, x1, x2):
        # Pad or crop x2 to match the spatial dimensions of x1 and concatenate
        diff_h = x1.size(2) - x2.size(2)
        diff_w = x1.size(3) - x2.size(3)

        if diff_h != 0 or diff_w != 0:
            x2 = F.pad(x2, (diff_w // 2, diff_w - diff_w // 2, diff_h // 2, diff_h - diff_h // 2))

        # Reduce the number of channels in x2 to match x1 before concatenation
        x2 = x2[:, :x1.size(1), :, :]
