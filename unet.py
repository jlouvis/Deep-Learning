...
images_128 = []
for image in images_tensors[0:100]: # only first 100 images - for testing
    # convert image to Float32 - required for interpolation
    image = image.type(torch.FloatTensor)
    image_128 = F.interpolate(image.unsqueeze(0), size=128, mode='bilinear', align_corners=False)
    images_128.append(image_128.squeeze(0))

# Reshape labels_tensors
labels_128 = []
for label in labels_tensors[0:100]: # only first 10 labels - for testing
    label_128 = F.interpolate(label.unsqueeze(0), size=128, mode='nearest')
    # convert label to LongTensor - required for CrossEntropyLoss
    label_128 = label_128.type(torch.LongTensor)
    labels_128.append(label_128.squeeze(0))

# remove channel dimension from labels - required for CrossEntropyLoss
labels_128 = [label[0] for label in labels_128]

# Normalize images_128
max_pixel = torch.max(torch.stack(images_128))
mean_pixel = torch.mean(torch.stack(images_128))
std_pixel = torch.std(torch.stack(images_128))
images_128_norm = []
for image in images_128:
    image_norm = (image - mean_pixel) / std_pixel
    images_128_norm.append(image_norm)

# Build data sets

# Build Tensor dataset
dataset = TensorDataset(torch.stack(images_128_norm), torch.stack(labels_128))
